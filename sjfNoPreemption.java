...
public class sjfNoPreemption {
	...
	static void findMaximumLength() {
		...
		for (int i = 0; i < candidates.size(); i++) {
			if (candidates.get(i).entireTime > Scheduler.processTable.get(maxLengthProcess).entireTime) {
	...
	static void decide() {
		...
		for (int i = 0; i < candidates.size(); i++) { // SZUKANIE KANDYDATA O
														// NAJMNIEJSZYM CZASIE
														// TRWANIA
			if (candidates.get(i).entireTime <= Scheduler.processTable.get(minLengthProcess).entireTime) {
	...
	public static void work() {
		System.out.println("Algorytm: SJF bez wywlaszczen");
		printProcesses();
			...
			if (candidates.size() > 0) {
				...
				while (!Scheduler.processTable.get(currentProcess).isFinished) {
					...
					if (Scheduler.processTable.get(currentProcess).timeRemaining == 0) {
						...
						System.out
								.println("Proces " + Scheduler.processTable.get(currentProcess).id + " sie skonczyl. Czas oczekiwania wyniosl "+Scheduler.processTable.get(currentProcess).waitingTime+ ".");
						finishedProcesses++;
